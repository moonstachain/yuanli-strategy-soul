...
const trilogyModules=[
  ['原力资产','A1','发现母体','#86','assets/A1-发现母体/',26,'Seed+','P1','worksheet.md + real-learner-cases.md'],
  ['原力资产','A2','回到母体','#87','assets/A2-回到母体/',24,'Seed','P1','calibration-examples.md + facilitator-notes.md'],
  ['原力资产','A3','获得原力','#88','assets/A3-获得原力/',25,'Seed+','P1','capability-rubric.md + validation-worksheet.md'],
  ['原力资产','A4','显化原力','#89','assets/A4-显化原力/',26,'Seed+','P0','validation-records.md + pricing-test.md'],
  ['原力创业','B1','原力借势','#90','assets/B1-原力借势/',27,'Seed+','P0','trend-map.md + industry-cases.md'],
  ['原力创业','B2','品类独创','#91','assets/B2-品类独创/',27,'Seed+','P0','naming-test.md + user-language.md'],
  ['原力创业','B3','模式升维','#92','assets/B3-模式升维/',27,'Seed+','P0','financial-template.md + delivery-sop.md'],
  ['原力创业','B4','壁垒锁定','#93','assets/B4-壁垒锁定/',27,'Seed+','P0','moat-diagnostic.md + metric-template.md'],
  ['原力OS','C1','一纸文脉','#94','assets/C1-一纸文脉/',26,'Seed+','P1','prompt-template.md + before-after-ai-output.md'],
  ['原力OS','C2','一个大脑','#95','assets/C2-一个大脑/',26,'Seed+','P1','tagging-template.md + retrieval-test.md'],
  ['原力OS','C3','一张地图','#96','assets/C3-一张地图/',25,'Seed+','P1','map-template.md + gap-scoring-sheet.md'],
  ['原力OS','C4','一条链路','#97','assets/C4-一条链路/',23,'Seed','P0','chain-template.md + operating-examples.md']
].map(([part,node,module,issue,asset_path,maturity,stage,priority,next_deepening])=>({part,node,module,issue,asset_path,status:'seed complete',maturity,stage,priority,evidence:'medium',next_deepening}));
const contentAssetItems=trilogyModules.map(m=>({issue:m.issue,node:m.node,type:'content_asset',domain:'trilogy',layer:'content',status:'completed',health:'green',priority:m.priority,gap_score:m.maturity,evidence:m.evidence,next:m.next_deepening}));
...
(async()=>{
  ...
  const obj={
    ...
    sync:{mode:'github_actions',source:'issues+content_assets',updated_at:new Date().toISOString()},
    ...
    views:[
      ...
      {id:'roadmap',name:'Roadmap / Milestone',layout:'roadmap',purpose:'manage release sequence',milestones:['v2.7.1 Data Sync Patch','v2.8 Interactive Six-view Drilldown','v2.9 Content Asset View','v3.0 PMO Metrics Engine','v3.1 Task Bridge','v3.2 Evidence Acceptance Loop','v3.3 Canonical Learning Gate','v3.4 Content Asset Sync v1']}
    ],
    metrics:['Evidence Coverage','Backwrite Rate','Evolution Note Rate','C3 Gap Close Rate','Blocked Age','Trilogy Seed Coverage','Average Asset Maturity','Validation Gap','P0 Deepening Count'],
    metric_values:{trilogy_seed_coverage:{value:100,unit:'%',note:'12/12 canon modules have seed content asset packages'},average_asset_maturity:{value:64,unit:'%',note:'309/480 scorecard baseline'},validation_gap:{value:1,unit:'/5',note:'real validation average remains the weakest link'},p0_deepening_count:{value:6,unit:'modules',note:'A4, B1, B2, B3, B4, C4'}},
    objects:['Portfolio','Project','Work Item','Gap','Task','Evidence','Evolution Note','Asset','Review','Approval Gate','Canonical Learning','Content Asset','Maturity Scorecard','Deepening Task'],
    automation:['issue changed -> sync dashboard','weekly review -> evolution note','selected gap -> C4 task draft','evidence review -> accepted/missing/rejected','accepted learning -> canonical candidate','human approval -> canonical learning','content asset changed -> update PMO content view','maturity scorecard updated -> generate deepening priorities'],
    content_sync:{name:'PMO Content Asset Sync v1',status:'completed',scope:['12 module registry','12 seed asset packages','content asset index','maturity scorecard','P0/P1 deepening priority'],reports:['docs/TRILOGY-CONTENT-ASSET-INDEX-v1.md','docs/TRILOGY-ASSET-MATURITY-SCORECARD-v1.md','docs/PMO-TRILOGY-CONTENT-SYNC-v1.md'],current_state:'PMO now covers engineering governance and trilogy content asset governance'},
    trilogy_summary:{parts:3,modules:12,seed_packages:12,coverage_pct:100,total_score:309,total_possible:480,average_score:'25.75/40',average_maturity_pct:64,seed_plus:10,seed:2,real_validation_average:'1/5'},
    ...
    roadmap_current:['v2.7.1','v2.8','v2.9-content-assets','v3.0','v3.1','v3.2','v3.3','v3.4-content-asset-sync-v1']
  };
  const js='window.YUANLI_PMO_V22='+JSON.stringify(obj,null,2)+';\n';
  fs.writeFileSync('docs/pmo-dashboard-data.js',js);
  console.log(`PMO dashboard synced: ${items.length} items, ${trilogyModules.length} content assets`);
})();
